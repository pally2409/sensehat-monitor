...
class coapClientConnector {
  public:             // Access specifier
    coapClientConnector(const char* host, const char* port) {     // Constructor
      host = host;
      port = port;
    ...
    void sendGET(std::string resource, bool confirmable, bool observe) {
      ...
      // coap_add_option(pdu, COAP_OPTION_URI_PATH, resource.size(), reinterpret_cast<const uint8_t *>(resource.c_str()));
      if(observe) {
